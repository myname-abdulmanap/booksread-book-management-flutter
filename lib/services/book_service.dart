...
class BookService {
  ...
  /// Get PDF books count
  Future<int> getPdfBooksCount() async {
    try {
      final response = await _supabase
          .from('books')
          .select()
      ...
      throw Exception('Failed to get PDF books count: ${e.toString()}');
    }
  }

  /// Get Text books count
  Future<int> getTextBooksCount() async {
    try {
      final response = await _supabase
          .from('books')
          .select()
      ...
      throw Exception('Failed to get text books count: ${e.toString()}');
    }
  }
